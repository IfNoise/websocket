...
export class JSONRPCwsServerWithDB {
  ...
  _onDeviceConnected(device) {
    ...
    try {
      ...
      device.call('Get.State', {})
        .then((result) => {
          DeviceService.updateDeviceState(device.deviceId, result);
          deviceLogger.rpcResponse(device.deviceId, 'Get.State', true, Date.now() - startTime);
        })
        .catch((err) => {
      ...
      deviceLogger.error(device.deviceId, err, { operation: 'device_connected' });
    }
  }
  ...
  _onDeviceNotification(device, frame) {
    ...
    // Например, обновления состояния или событий
  ...
  _startStatusMonitor() {
    this.statusInterval = setInterval(() => {
      ...
      devices.forEach(device => {
        ...
        try {
          const status = device.ws?.readyState === 1 ? 'connected' : 'disconnected';
