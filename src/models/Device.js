...
export class DeviceModel {
  ...
  static updateConfig(id, config) {
    ...
    const oldConfigStr = currentDevice?.config
  ...
  static updateState(id, state) {
    ...
    const oldStateStr = currentDevice?.state
      ? normalizeState(currentDevice.state)
      : null;
    ...
    const stmt = db.prepare(
      "UPDATE devices SET state = ?, last_seen = ? WHERE id = ?",
    );
